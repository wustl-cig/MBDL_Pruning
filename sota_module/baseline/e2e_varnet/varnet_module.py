...
from .varnet import VarNet, E2EVarNet

from .mri_module import MriModule
#from method.DeCoLearn import inputDataDictVarnet

from torch.utils.data import Dataset

class inputDataDictE2EVarnet(Dataset):
...
class inputDataDictVarnet(Dataset):
    def __init__(self, masked_kspace, mask, S, num_low_frequencies=None):
        self.masked_kspace = masked_kspace
        self.mask = mask
        self.num_low_frequencies = num_low_frequencies
        self.S = S
    ...
    def getData(self):
        return self.masked_kspace, self.mask, self.S, self.num_low_frequencies
...
class VarNetModule(MriModule):
    ...
    def forward(self, XPSY):
        ...
        masked_kspace = y
        mask = P
        INPUT_VARNET = inputDataDictVarnet(masked_kspace, mask, S=S, num_low_frequencies=None)
        x_recorvered = self.varnet(INPUT_VARNET)

        x_recorvered = x_recorvered.permute([0, 3, 1, 2]).contiguous()
        return x_recorvered
...
class E2EVarNetModule(MriModule):
    """
    VarNet training module.
    ...
    def forward(self, XPSY):
        ...
        masked_kspace = y
        mask = P
        INPUT_VARNET = inputDataDictE2EVarnet(masked_kspace, mask, num_low_frequencies=None)
        [x_recorvered, estimated_sen_map] = self.varnet(INPUT_VARNET)

        x_recorvered = x_recorvered.permute([0, 3, 1, 2]).contiguous()
        return x_recorvered, estimated_sen_map
