...
def run(config):
    ...
    model = ImageWarmup(
        net=get_module_from_config(config, type_='theta', use_sigma_map=(config['method']['deq_cal']['warmup']['theta_ckpt'] == 'g_denoise')),
        sigma=config['method']['deq_cal']['warmup']['theta_sigma'],
        lr=1e-4,
        is_g_denoise=(config['method']['deq_cal']['warmup']['theta_ckpt'] == 'g_denoise')
    )
