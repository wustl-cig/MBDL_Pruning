...
class DEQCalibrationMRI(pl.LightningModule):
    ...
    def forward(self, x_init, theta_init, mask, y):
        ...
        return x_hat, theta_hat, res_x_list, res_theta_list, res_list
    ...
    def step_helper(self, batch):
        ...
        x_hat, theta_hat, res_x_list, res_theta_list, res_list = self(x_input, theta_input, mask, y)
        ...
        return loss, psnr, ssim, theta_mse, x_input, x_hat, x_gt, theta_input, theta_hat, theta_gt, res_x_list, res_theta_list, res_list

    def test_step(self, batch, batch_idx):
        loss, psnr, ssim, theta_mse, x_input, x_hat, x_gt, theta_input, theta_hat, theta_gt, res_x_list, res_theta_list, res_list = self.step_helper(batch)
    ...
    def theta_mse_helper(theta_hat, theta_gt):
        ...
        return torch.norm(theta_hat - theta_gt) / torch.norm(theta_gt)
...
def run(config):
    ...
    if config['setting']['mode'] == 'dug':
        tst_dataset = Subset(tst_dataset, [9, 59])
    else:
        tst_dataset = Subset(tst_dataset, [0, 9, 19, 29, 39, 49, 59, 69, 79, 89])
