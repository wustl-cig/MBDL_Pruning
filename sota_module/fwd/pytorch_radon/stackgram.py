...
class Stackgram(nn.Module):
    ...
    def forward(self, x):
        stackgram = torch.zeros(x.shape[0], len(self.theta), self.in_size, self.in_size, device=x.device,
                                dtype=self.dtype)

        for i_theta in range(len(self.theta)):
            repline = x[..., i_theta]
            repline = repline.unsqueeze(-1).repeat(1, 1, 1, repline.shape[2])
            linogram = grid_sample(repline, self.all_grids[i_theta].repeat(x.shape[0], 1, 1, 1).to(x.device),
                                   mode=self.mode)
...
class IStackgram(nn.Module):
    ...
    def forward(self, x):
        ...
        for i_theta in range(len(self.theta)):
            ...
            repline = grid_sample(linogram, self.all_grids[i_theta].repeat(x.shape[0], 1, 1, 1).to(x.device),
                                  mode=self.mode)
