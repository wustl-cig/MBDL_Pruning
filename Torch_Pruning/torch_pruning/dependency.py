...
class DependencyGraph(object):
    ...
    def build_dependency(
        ...
        """
        #masked_kspace, mask, num_low_frequencies = example_inputs.getData()
        #print(f"masked_kspace.shape: {masked_kspace.shape}")
        #print(f"mask.shape: {mask.shape}")
        #ValueError(f"num_low_frequencies.shape: {num_low_frequencies.shape}")
        #print("1")
        ...
        for p in unwrapped_detected:
            ...
            def last_non_singleton_dim(tensor):
                ...
                #print(f"non_singleton_dims: {non_singleton_dims}")
        ...
        #print(f"After 5: {len(self.unwrapped_parameters)}")
        print("6")
        #print(self.unwrapped_parameters)
    ...
    def _infer_out_channels_recursively(self, node: Node):
        ...
        #print(f"[dependency][_infer_out_channels_recursively] type(ch): {type(ch)}")
        if ch is None:
            #print("I AM IN")
            ch = 0
            #print(f"[dependency][_infer_out_channels_recursively] len(node.inputs): {len(node.inputs)}")
            for in_node in node.inputs:
                #print(f"[dependency][_infer_out_channels_recursively] node.type: {node.type}")
                #print(f"[dependency][_infer_out_channels_recursively] ops.OPTYPE.CONCAT: {ops.OPTYPE.CONCAT}")
                if node.type == ops.OPTYPE.CONCAT:
                    ...
                    #print(f"[dependency][_infer_out_channels_recursively] in_node.type: {in_node.type}")
                    #print(f"[dependency][_infer_out_channels_recursively] in_node.module.split_sizes: {in_node.module.split_sizes}")
    ...
    def _trace(self, model, example_inputs, forward_fn, output_transform):
        ...
        if len(self.unwrapped_parameters) > 0:
            ...
            #print("VISIT1")
            #print("VISIT1")
            #print(f"VISIT: {module2node.values()}")
    ...
    def _trace_computational_graph(self, module2node, grad_fn_root, gradfn2module, reused):
        #print(f"_trace_computational_graph: {module2node.values()}")
        #print(f"_trace_computational_graph: {module2node.values()}")
        #print(f"_trace_computational_graph: {module2node.values()}")
        #print(f"_trace_computational_graph: {module2node.values()}")
        ...
        while len(processing_stack) > 0:
            ...
            #print(f"[2]_trace_computational_graph: {module2node.values()}")
        ...
        #print(f"[3]KEY: {module2node.keys()}")
        #print(f"[3]VALUES: {module2node.values()}")
    ...
    def update_index_mapping(self):
        ...
        for module, node in self.module2node.items():
            ...
            #print(f"[update_index_mapping]node: {node}")
            #print(f"[update_index_mapping]node.type: {node.type}")
            if node.type == ops.OPTYPE.CONCAT:
                #print(f"1: {node.type}")
        ...
        #print(f"4: Finish update_index_mapping")
    ...
    def _update_reshape_index_mapping(self, reshape_node: Node):
        #print(f"[_update_reshape_index_mapping]: {reshape_node}")
        ...
        if hasattr(reshape_node.grad_fn, '_saved_self_sizes'):
            ...
            if not self._2d_4d:
                return
            return # chicago add this to make Varnet & ISTANet work
    ...
    def infer_channels(self, node_1, node_2):
        if node_1.type == ops.OPTYPE.SPLIT:
            for i, n in enumerate(node_1.outputs):
                if n == node_2:
                    #print(f"[dependency][infer_channels]first return node_1.type: {node_1.type}")
                    return node_1.module.split_sizes[i]
        #print(f"[dependency][infer_channels]second return node_1.type: {node_1.type}")
